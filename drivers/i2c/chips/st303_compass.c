/* linux/drivers/i2c/chips/st303_compass.c
 *
...
int st303_probe(struct i2c_client *client, const struct i2c_device_id * devid)
{
	...
	rc = i2c_smbus_write_byte_data(client,CRB_REG_M,0x40);//0x20 1.3gauss 0xe0 8.1gauss
	rc = i2c_smbus_write_byte_data(client,MR_REG_M, 0x00);
	if (rc < 0){
		err = -ENODEV;
