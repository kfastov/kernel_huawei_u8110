...
#else
#ifdef CONFIG_USB_AUTO_INSTALL
static int usb_pid = PID_ONLY_CDROM; 
#else
static int pid = 0x1502; 
#endif
#endif
...
{
    USB_PR("lxy: %s, usb_para_info.usb_pid=0x%x\n", __func__, usb_para_info.usb_pid);
...
#ifdef CONFIG_USB_AUTO_INSTALL
    USB_PR("lxy: set pid in usb_try_to_bind\n");
#endif
...
static unsigned short usb_set_composition(unsigned short pid)
{
	...
	for (i = 0; i < ui->pdata->num_compositions; i++) {
		if (ui->pdata->compositions[i].product_id == pid) {
            ...
            USB_PR("lxy: %s, composition set to product id = %x\n",
				__func__, ui->composition->product_id);
			return ui->composition->product_id;
		}
	}
	pr_err("%s: product id (%x) not supported\n", __func__, pid);
    USB_PR("lxy: %s, product id (%x) not supported\n", __func__, pid);
	return 0;
}
...
#endif
{
...
#ifdef CONFIG_USB_AUTO_INSTALL
    USB_PR("lxy: usb_set_composition\n");
#endif
	usb_configure_device_descriptor(ui);
    ...
    USB_PR("lxy: %s, usb_switch_para.inprogress = 0\n", __func__);
...
static void usb_do_work(struct work_struct *w)
{
	...
	for (;;) {
		...
		if (flags == 0)
			break;
#ifdef CONFIG_USB_AUTO_INSTALL
		USB_PR("lxy: %s: ui->state=%d, flags=0x%x\n", __func__, ui->state, flags);
#endif
		switch (ui->state) {
		case USB_STATE_IDLE:
