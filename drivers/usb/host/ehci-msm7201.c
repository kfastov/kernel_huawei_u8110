/*
 * Huawei U8150 Ideos support added by Sven Killig <sven@killig.de>
 *
...
static void msm7201_usb_setup(struct usb_hcd *hcd)
{
	...
	//	msm7201->phy_reset(hcd->regs);

	//waiting 100 ms after phy_reset
	msleep(100);
...
static int usb_hcd_msm7201_probe(struct platform_device *pdev)
{
	...
	struct msm_usb_host_platform_data *pdata = pdev->dev.platform_data;
...
msm_chg_usb_charger_connected(CHG_TYPE_HOSTPC);
